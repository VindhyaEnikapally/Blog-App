...
authorRoute.post("/users", upload.single("profileImageUrl"), async (req, res, next) => {
  ...
  try {
    ...
    //  Step 1: upload image to cloudinary from memoryStorage (if exists)r
    if (req.file) {
...
authorRoute.get("/articles/:authorId", verifyToken("AUTHOR"), async (req, res) => {
  ...
  let articles = await ArticleModel.find({ author: aid }).populate("author", "firstName email").populate("comments.user", "firstName email");
  //send res
